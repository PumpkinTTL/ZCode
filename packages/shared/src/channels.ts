...
export const PlatformChannels = {
  ...
  /** Renderer → Main：写入宿主 ~/.polaris 临时文本附件 */
  ...
  /** 资源管理器「存储」tab：开始扫描本机 .polaris 占用（main 持有 StorageService，Worker 线程遍历） */
  ...
  /** Renderer → Main：导出日志（打包 ~/.polaris/v2 及外部 agent 日志为 zip 并在 Finder 中显示） */
