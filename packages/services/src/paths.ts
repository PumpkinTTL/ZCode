...
/** 用户数据根目录名。Polaris fork：`.zcode` → `.polaris`（不做旧目录自动迁移，干净起步）。 */
export const DATA_ROOT_DIR_NAME = ".polaris";

/** {dataBaseDir}/.polaris */
export function getZCodeDataRootDir(): string {
  return join(getDataBaseDir(), DATA_ROOT_DIR_NAME);
}

/** 非项目对话共享的真实工作目录；默认 ~/.polaris/workspace/default。 */
export function getConversationWorkspaceDir(): string {
  return join(getZCodeDataRootDir(), "workspace", "default");
}

/** {dataBaseDir}/.polaris/v2 */
...
/** ~/.polaris/v2/sessions/{workspaceHash} */
...
/** ~/.polaris/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.polaris/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .polaris/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, DATA_ROOT_DIR_NAME, "v2");
  const newDir = join(newBaseDir, DATA_ROOT_DIR_NAME, "v2");
