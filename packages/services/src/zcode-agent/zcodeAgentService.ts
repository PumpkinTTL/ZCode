...
import { DATA_ROOT_DIR_NAME, getDataBaseDir } from "../paths.js";
import {
...
function ensurePluginManagementWorkspacePath(): string {
  // 控制面 workspace 直接落在数据根下（Polaris fork：`.polaris`），不要再嵌一层 `.zcode`。
...
// CLI/bootstrap MCP servers configured in ~/.polaris/cli/config.json (mcp.servers), which the agent
...
export function createZCodeAgentService(
  ...
  // homedir() 即用户家目录，全局根 `~/.polaris/workflows/` 因此解析到真实目录。
