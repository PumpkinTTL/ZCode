...
import {
  ...
  buildApiKeySetupSkipSettings,
  resolveApiKeySetupDefaultProvider,
  resolveApiKeySetupProviderLabel,
  resolveApiKeySetupTemplateId,
  shouldShowApiKeySetupLink,
  type ApiKeyProviderChoice,
} from "@/provider-setup/ApiKeySetupForm.helpers.js";
...
export function ApiKeySetupForm({ onCancel, onSaved, onSkipped }: ApiKeySetupFormProps) {
  ...
  const providerLabel = resolveApiKeySetupProviderLabel(providerChoice);
  const templateId = resolveApiKeySetupTemplateId(providerChoice);
  ...
  const showApiKeyLink = shouldShowApiKeySetupLink(apiKeyValue, apiKeyUrl ?? undefined);
  ...
  const saveApiKeyProvider = async () => {
    ...
    try {
      ...
      const defaultModelPreference = buildApiKeySetupDefaultModelPreferenceFromSelection(
      ...
      logger.error("[ApiKeySetup] 保存 API Key provider 失败", {
        templateId,
        error: saveError,
      });
      setError(
  ...
  const skipApiKeyProvider = async () => {
    ...
    try {
      ...
      await settingService.update(buildApiKeySetupSkipSettings(providerChoice, Date.now()));
      await onSkipped();
    } catch (skipError) {
      logger.error("[ApiKeySetup] 跳过 API Key 设置失败", {
        providerChoice,
        error: skipError,
      });
      setError(
